...
def validate_counterarguments(
    ...
    for counter in counterarguments:
        ...
        errors.extend(
            check_forbidden_phrases(
                counterargument_id, counter.get("counterargument_text"),
...
def validate_rebuttals(
    ...
    for rebuttal in rebuttals:
        ...
        errors.extend(
            check_forbidden_phrases(
                rebuttal_id, rebuttal.get("rebuttal_text"),
...
def validate_agent_suggestions(
    ...
    for suggestion in suggestions:
        ...
        errors.extend(
            check_forbidden_phrases(
                suggestion_id, suggestion.get("grounded_explanation")
            )
        )

        # --------------------------------------------------------
        # SUGGESTION FREE-TEXT SAFETY - INDEPENDENT RE-CHECK
        # (Finding 2 remediation). Bu, argument_agent.py'nin
        ...
        if not isinstance(grounded_explanation, str) or not grounded_explanation.strip():
            ...
            errors.append(f"{suggestion_id}: grounded_explanation boş olamaz.")

        elif len(grounded_explanation) > MAX_GROUNDED_EXPLANATION_LENGTH:

            errors.append(
                f"{suggestion_id}: grounded_explanation uzunluk sınırını "
                "aşıyor."
            )
            ...
            smuggled = find_smuggled_ids(
                grounded_explanation, declared_ids, known_reference_ids
            )

            if smuggled:

                errors.append(
                    f"{suggestion_id}: grounded_explanation içine "
                    f"bilinmeyen/deklare edilmemiş ID gömülü: {smuggled}"
                )
            ...
            unverified_quotes = find_unverified_quotes(
                grounded_explanation, citable_texts
            )

            if unverified_quotes:

                errors.append(
                    f"{suggestion_id}: grounded_explanation içindeki "
                    "alıntı referans kaynaklarda birebir doğrulanamadı: "
                    f"{unverified_quotes}"
                )

            unsupported = find_unsupported_numeric_tokens(
                grounded_explanation, citable_texts
            )
            ...
            if unsupported:
                ...
                errors.append(
                    f"{suggestion_id}: grounded_explanation içindeki "
                    "tarih/tutar referans kaynaklarda birebir bulunamadı "
...
def run_self_test(case_id):
    ...
    print()
    print("======================================")
    print(" ARGUMENT VALIDATOR V1: 17/17 PASS")
    print("======================================")
