...
def write_review_audit(
    ...
    *,
    ...
    audit = {
        ...
        "case_id": case_id,
        # ROW 19C-2b: additive, keyword-only, default None - mirrors
        # Row 19C-2a's own `mutation_idempotency_key`/`mutation_resource_key`
        # on the 10 Layer A `write_approval_audit()` functions.
        # `mutation_actor_ref` carries the REAL actor identity
        # (`MutationIntent.actor_ref`) - deliberately separate from the
        # FIXED, non-identity `reviewer_ref` sentinel below.
...
def apply_review_transition(
    ...
    *,
    ...
    # state/parent-dependency guards, backup/atomic-write/rollback) is
    # completely UNCHANGED. Both default to None, preserving this
    # function's exact pre-Row-19C-2b behavior/CLI output.
