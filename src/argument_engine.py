...
GENERATION_ACTION_FAMILY = "generation.argument"
...
def get_reviews_dir(case_id):
    ...
    return get_arguments_dir(case_id) / "generation_reviews"
...
def write_carry_forward_audit(case_id, carry_records, *, carry_forward_dir=None):
    """ROW 19C-3c-ii: `carry_forward_dir` additive, keyword-only,
    varsayılan `None` - legacy davranış (kendi ham `get_carry_forward_
    dir(case_id)` getter'ını çağırır) byte-for-byte KORUNUR. Coordinated
    path facade'in lock-altında doğrulamış olduğu `VerifiedGeneration
    OutputPaths.carry_forward_dir`'i açıkça geçirir."""
    ...
    carry_dir = (
        carry_forward_dir if carry_forward_dir is not None
        else get_carry_forward_dir(case_id)
    )
...
def build_argument_engine_output(
    ...
    *,
    write_carry_forward_audit_enabled=True,
):
    """ROW 19C-3c-ii: `write_carry_forward_audit_enabled` additive,
    keyword-only, varsayılan `True` - legacy/default davranış (bu
    fonksiyon carry-forward audit'i KENDİSİ, dış-lock build sırasında
    yazar) byte-for-byte KORUNUR. Coordinated path (ui.services.
    agent_generation_mutation_facade) bunu AÇIKÇA `False` geçirir:
    build outside-lock, ZERO filesystem mutation olur; `carry_records`
    (ham liste - artık return dict'inde de mevcut) facade tarafından
    dondurulup under-lock write_pending()'e taşınır, carry-forward
    audit'i YALNIZ orada, başarılı pending commit sınırında üretilir."""
...
def write_pending(
    ...
    *,
    ...
    AYNI additive/keyword-only/legacy-preserving gerekçesi. `argument`'a
    özgü ek: `carry_records` (facade'in frozen bytes'tan `json.loads()`
    ile yeniden ürettiği taze liste) verilmişse ve boş değilse, coordi-
    nator success audit'inden ÖNCE `write_carry_forward_audit()` (var
    olan, DEĞİŞTİRİLMEMİŞ fonksiyon) çağrılır; ardından coordinator
    audit'i buna bağlanır (`carry_forward_audit_path`/`_sha256`). İkinci
    yazma (coordinator audit) başarısız olursa, İLK yazılan carry-
    forward audit dosyasını silmek için best-effort bir temizlik
    denenir (ikincil hata yalnız loglanır, asla orijinal exception'ı
    MASKELEMEZ); ardından write_pending'in KENDİ dış rollback'i
    (pending unlink + backup restore) normal şekilde devam eder."""
    ...
    if verified_paths is not None:
        ...
        arguments_dir = verified_paths.family_root
        ...
        carry_forward_dir_override = verified_paths.carry_forward_dir
        ...
        carry_forward_dir_override = None
    ...
    try:
        ...
        if mutation_binding_provided:
            ...
            carry_forward_audit_sha256 = None

            if carry_records:

                carry_forward_audit_path = write_carry_forward_audit(
                    case_id, carry_records,
                    carry_forward_dir=carry_forward_dir_override,
                )

                carry_forward_audit_sha256 = hashlib.sha256(
                    carry_forward_audit_path.read_bytes()
                ).hexdigest()
            ...
            audit_record = {
                ...
                "carry_forward_audit_path": (
                    str(carry_forward_audit_path)
                    if carry_forward_audit_path is not None else None
                ),
                "carry_forward_audit_sha256": carry_forward_audit_sha256,
            ...
            try:
                ...
                if carry_forward_audit_path is not None:
                    ...
                    try:
                        ...
                        if carry_forward_audit_path.exists():

                            carry_forward_audit_path.unlink()
                        ...
                        logging.critical(
                            "ROW 19C-3c-ii argument: carry-forward audit "
                            "best-effort cleanup FAILED after coordinator "
                            "audit write failure (orphan may remain): %s",
                            cleanup_error,
                        )
...
def main():
    ...
    # generation --row-key argument ...` üzerinden yaşar. run_engine()/
    # write_pending() KENDİLERİ DEĞİŞTİRİLMEDİ - yalnız BU executable
