...
def _default_registry_factory() -> mr.MutationAdapterRegistry:
    ...
    ROW 19C-3c-i / ROW 19C-3c-ii: builds ONE merged registry covering
    EVERY production mutation family this coordinator serves - Layer A's
    10 case-scoped approval families (`mutation_approval_adapters.
    build_production_registry()`) PLUS Layer B's 12 review_kinds under 24
    channel-separated routing keys (`review_mutation_adapters.
    register_into()`) PLUS Row 18C's single `drafting_request.save`
    family (`drafting_request_mutation_adapters.register_into()`) PLUS
    Slice 2's two fact/timeline promotion families (`promotion_mutation_
    adapters.register_into()`) PLUS Row 19C-3c-i's two deadline/timeline
    deterministic pending-generation families (`generation_mutation_
    adapters.register_into()`) PLUS Row 19C-3c-ii's five case-scoped
    agent-gated pending-generation families - issue_spotting/evidence/
    argument/risk_strategy/drafting (`agent_generation_mutation_
    adapters.register_into()`, a SEPARATE facade/adapter pair from the
    deterministic-only `generation_mutation_adapters` above, NOT an
    extension of it) - 10 + 24 + 1 + 2 + 2 + 5 = 44 routing keys, all
    ...
    produces, regardless of family, through this ONE CLI. NEVER called
    by `ui/tests/test_reconciliation_operator_isolated.py` (which always
    ...
    return agent_generation_mutation_adapters.register_into(registry)
